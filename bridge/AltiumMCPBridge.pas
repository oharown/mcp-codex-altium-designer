...
const
    ...
    { 120 ticks at 250 ms gives a 30 second bridge window. }
    MaxPollTicks = 120;
...
function RunProjectValidationJson : String;
var
    ActivatedSchematic : Boolean;
begin
    ActivatedSchematic := False;

    try
        ActivatedSchematic := ActivateFirstWorkspaceDocumentByKind('SCH');
    except
        ActivatedSchematic := False;
    end;
    ...
    try
        ...
        Result := '{' +
            JsonString('process', 'WorkspaceManager:Compile') + ',' +
            JsonString('status', 'dispatched') + ',' +
            JsonBool('activated_schematic', ActivatedSchematic) + ',' +
            JsonString('note', 'Review validation/ERC messages in Altium Messages panel.') + '}';
    except
        Result := '{' +
            JsonString('process', 'WorkspaceManager:Compile') + ',' +
            JsonString('status', 'error') + ',' +
            JsonString('error', 'Unable to dispatch project validation.') + '}';
    end;
end;


function OpenPcbDrcDialogJson : String;
var
    Board : Variant;
    ActivatedPcb : Boolean;
begin
    ActivatedPcb := False;
    ...
    begin
        Result := '{' +
            JsonString('process', 'PCB:DesignRuleCheck') + ',' +
            JsonString('status', 'error') + ',' +
            JsonString('error', 'No current PCB board.') + '}';
        Exit;
    end;
    ...
    try
        ...
        Result := '{' +
            JsonString('process', 'PCB:DesignRuleCheck') + ',' +
            JsonString('status', 'dialog_opened') + ',' +
            JsonBool('activated_pcb', ActivatedPcb) + ',' +
            JsonString('note', 'Review settings and run Batch DRC from the Altium dialog.') + '}';
    except
        Result := '{' +
            JsonString('process', 'PCB:DesignRuleCheck') + ',' +
            JsonString('status', 'error') + ',' +
            JsonString('error', 'Unable to open PCB Design Rule Check dialog.') + '}';
    end;
end;


function RunActiveOutputContainerJson(Mode : String) : String;
var
    ProcessName : String;
    ActionName : String;
    ObjectKind : String;
begin
    ...
    begin
        ...
        ActionName := 'PublishToPDF';
        ObjectKind := 'OutputBatch';
    end
    else
    begin
        ProcessName := 'WorkspaceManager:GenerateReport';
        ActionName := 'Run';
        ObjectKind := 'OutputBatch';
    end;
    ...
    try
        ResetParameters;
        AddStringParameter('Action', ActionName);
        AddStringParameter('ObjectKind', ObjectKind);
        if Mode = 'pdf' then
            AddStringParameter('DisableDialog', 'True');
        RunProcess(ProcessName);

        Result := '{' +
            JsonString('process', ProcessName) + ',' +
            JsonString('status', 'dispatched') + ',' +
            JsonString('mode', Mode) + ',' +
            JsonString('note', 'Outputs are generated by the active OutJob output container.') + '}';
    except
        Result := '{' +
            JsonString('process', ProcessName) + ',' +
            JsonString('status', 'error') + ',' +
            JsonString('mode', Mode) + ',' +
            JsonString('error', 'Unable to dispatch active OutJob output generation.') + '}';
    end;
end;
...
begin
    ...
    else if Command = 'run_project_validation' then
        Result := RunProjectValidationJson
    else if Command = 'open_pcb_drc_dialog' then
        Result := OpenPcbDrcDialogJson
    else if Command = 'run_active_output_container' then
        Result := RunActiveOutputContainerJson(ExtractJsonString(ReadTextFile(RequestFile), 'mode'))
    else
        Result := '';
end;
